...
import { JsonValue, schema } from '@nger/cli.json';
import {
  Job,
  JobDescription,
  JobHandler,
...
export class SimpleScheduler<
  ...
  private _getInternalDescription(name: JobName): Observable<JobHandlerWithExtra | null> {
    ...
    return handler.pipe(
      switchMap(handler => {
        ...
        this._internalJobDescriptionMap.set(name, handlerWithExtra as any);

        return of<any>(handlerWithExtra);
      }),
    );
  }
  ...
  private _createJob<A extends MinimumArgumentT, I extends MinimumInputT, O extends MinimumOutputT>(
    ...
    const output = outboundBus.pipe(
      filter<any>(x => x.kind === JobOutboundMessageKind.Output),

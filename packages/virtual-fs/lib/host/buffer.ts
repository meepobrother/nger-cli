import { tags } from '@nger/cli.utils';
...
export const fileBuffer: tags.TemplateTag<FileBuffer> = (strings, ...values) => {
...
export function fileBufferToString(fileBuffer: FileBuffer): string {
    if (fileBuffer.toString.length == 1) {
        ...
        for (let i = 0; i < bufLength; i += chunkLength) {
            ...
            result += String.fromCharCode.apply(null, bufView.subarray(i, i + chunkLength) as any);
        }
        return result;
    }
}

...
export class NodeJsAsyncHost implements virtualFs.Host<fs.Stats> {
    ...
    read(path: Path): Observable<virtualFs.FileBuffer> {
        return _callFs(fs.readFile as any, getSystemPath(path)).pipe(
            map((buffer: any) => new Uint8Array(buffer).buffer as virtualFs.FileBuffer),
        );
    }
                ...
                if (isDirectory) {
                    ...
                    return concat(
                        observableFrom(allFiles).pipe(
                            mergeMap(p => _callFs(fs.unlink as any, getSystemPath(p))),
                            ignoreElements(),
                        ),
                        observableFrom(allDirs).pipe(
                            concatMap(p => _callFs(fs.rmdir as any, getSystemPath(p))),
                        ),
                    );
                } else {
                    return _callFs(fs.unlink as any, getSystemPath(path));
                }
            }),
            map(() => undefined),
        );
    ...
    rename(from: Path, to: Path): Observable<void> {
        return _callFs(fs.rename as any, getSystemPath(from), getSystemPath(to));
    }

    list(path: Path): Observable<PathFragment[]> {
        return _callFs(fs.readdir as any, getSystemPath(path)).pipe(
            map<any, PathFragment[]>((names: string[]) => names.map(name => fragment(name))),
        );
    }
    ...
    isDirectory(path: Path): Observable<boolean> {
        return _callFs(fs.stat as any, getSystemPath(path)).pipe(
            map((stat: any) => stat.isDirectory()),
        );
    }
    isFile(path: Path): Observable<boolean> {
        return _callFs(fs.stat as any, getSystemPath(path)).pipe(
            map((stat: any) => stat.isFile()),
        );
    }

    // Some hosts may not support stat.
    stat(path: Path): Observable<virtualFs.Stats<fs.Stats>> | null {
        return _callFs(fs.stat as any, getSystemPath(path));
    }
